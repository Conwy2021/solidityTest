...
contract MasterChef is Ownable {// conwy  begin
    ...
    uint256 public sushiPerBlock;//合约中貌似是100 个代币 100*10**18
    ...
    // View function to see pending SUSHIs on frontend. 计算待收益的奖励币的数量
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSushiPerShare).div(1e12).sub(user.rewardDebt);//总收益 减去之前已得到的收益
    ...
    // Update dev address by the previous dev. 修改开发者地址
