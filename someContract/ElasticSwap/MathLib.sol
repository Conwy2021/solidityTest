...
library MathLib {
    ...
    struct TokenQtys {
        uint256 baseTokenQty;//基础代币
        uint256 quoteTokenQty;//相对代币
        uint256 liquidityTokenQty;//流动性
        uint256 liquidityTokenFeeQty;// 流动性费用
    }
    ...
    function isSufficientDecayPresent(
        uint256 _baseTokenReserveQty,//baseToken 在exchange合约的数量
    ...
    function calculateAddLiquidityQuantities(
        ...
        uint256 _baseTokenReserveQty,//baseToken 在exchange合约的数量
        uint256 _totalSupplyOfLiquidityTokens,//此时的流动性代币总量
        InternalBalances storage _internalBalances
        ...
        if (_totalSupplyOfLiquidityTokens != 0) {
            ...
            // this user will set the initial pricing curve 第一次添加流动性
            require(
        ...
        )
    {
        ...
        liquidityTokenQty = calculateLiquidityTokenQtyForDoubleAssetEntry(//添加流动行 实际就是按投入的数量/_internalBalancesd的数量 的占比
            _totalSupplyOfLiquidityTokens,
            quoteTokenQty,
            _internalBalances.quoteTokenReserveQty
        );
