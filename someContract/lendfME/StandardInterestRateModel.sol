...
contract StandardInterestRateModel is Exponential {
    ...
    function getUtilizationRate(uint cash, uint borrows) pure internal returns (IRError, Exp memory) {
        ...
        (Error err1, Exp memory utilizationRate) = getExp(borrows, cashPlusBorrows);//返回的是资金利用率 结果放大10**18
    ...
    function getUtilizationAndAnnualBorrowRate(uint cash, uint borrows) pure internal returns (IRError, Exp memory, Exp memory) {
        ...
        // 45% of utilizationRate, is `rate * 45 / 100`  // conwy 45/100 可以理解成k值
        ...
        return (IRError.NO_ERROR, utilizationRate, annualBorrowRate);// 返回的是资金利用率和借款利率
    }
    ...
    function getSupplyRate(address _asset, uint cash, uint borrows) public view returns (uint, uint) {
        ...
        (Error err1, Exp memory utilizationRate1) = mulScalar(utilizationRate0, oneMinusSpreadBasisPoints);//oneMinusSpreadBasisPoints 9000 根据白皮书描述 0.1是给协议的 也叫利差
