...
contract StableCoinInterestRateModel is Exponential {
    ...
    function getUtilizationAndAnnualBorrowRate(uint cash, uint borrows) pure internal returns (IRError, Exp memory, Exp memory) {
        ...
        // utilizationRate * 0.3 (30%), which is `rate * 3 / 10` //conwy 0.3 的意思是 k
        ...
        (Error err3, Exp memory annualBorrowRate) = addExp(utilizationRateScaled, Exp({mantissa: mantissaOneTenth}));// mantissaOneTenth 基础利率
    ...
    function getSupplyRate(address _asset, uint cash, uint borrows) public view returns (uint, uint) {
        ...
        if (err0 != IRError.NO_ERROR) {//返回的资金使用率 借贷利率
            return (uint(err0), 0);
        }

        // We're going to multiply the utilization rate by the spread's numerator
        (Error err1, Exp memory utilizationRate1) = mulScalar(utilizationRate0, oneMinusSpreadBasisPoints);//oneMinusSpreadBasisPoints 8500 这个可能是安全系数 资金使用的85%
