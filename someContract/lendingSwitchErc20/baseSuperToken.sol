...
abstract contract baseSuperToken is timeLockSetting,superTokenInterface,proxyOwner,ReentrancyGuard{
    ...
    function _deposit(address from,uint256 _amount, address receiver) internal returns (uint256){
        ...
        uint256 totaStake = getTotalAssets();// conwy 抵押时这个值变小了  按翻译是资产
        // Gets the amount of superToken in existence
        uint256 totalShares = totalSupply(); //按翻译是股份 应该是LP的数量
        _amount = onDeposit(from,_amount,feeRate[enterFeeID]);//这一步  将钱 转给LSWUSDC  后LSWUSDC再把钱投到Aave: Pool V3 返回代币数量
    ...
    function flashLoan(//conwy flashLoan函数
