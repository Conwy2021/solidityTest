...
abstract contract superAaveTokenImpl is superTokenInterface {
    ...
    function aaveSupply(uint256 _feeRate)internal returns(uint256){
        uint256 balance = asset.balanceOf(address(this));// 计算合约的usdc
        if (balance>0){
            uint256 fee = balance.mul(_feeRate)/calDecimals;// fee 是 0
            if (fee > 0){
    ...
    function onDeposit(address account,uint256 _amount,uint64 _fee)internal virtual override returns(uint256){//conwy 调用这个实现地址
        asset.safeTransferFrom(account, address(this), _amount);
        return aaveSupply(_fee);
    }
    function onWithdraw(address account,uint256 _amount)internal virtual override returns(uint256){//conwy  调用的是这个 onWithdraw函数
