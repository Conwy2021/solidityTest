...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        uint32 blockTimestamp = uint32(block.timestamp % 2**32);//2**32=4,294,967,296 时间戳为32位存储 uint32截取 和取模都是为了防止溢出。貌似作者表示多写了部分原因https://github.com/Uniswap/v2-core/issues/96
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // overflow is desired
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // * never overflows, and + overflow is desired
            price0CumulativeLast += uint(UQ112x112.encode(_reserve1).uqdiv(_reserve0)) * timeElapsed;//_reserve1/_reserve0 计算的就是上一个区块最后一笔交易价格
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        } else {//todo MINIMUM_LIQUIDITY 限制了 _totalSupply 最小为1001 _totalSupply可以与_reserve0约分 降低1000倍
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {// uint amount0Out 0 , uint amount1Out 是 50000000000000000000
        ...
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));//balance0Adjusted = 4100*1000 - 100*3 = 4100000-300 = 4099700等同于4000+0.997*100
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));//balance1Adjusted=3903*1000-0 = 3903000
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'UniswapV2: K');
        //balance0Adjusted*balance1Adjusted =  16001129.1* 1000* 1000   >   _reserve0*_reserve1*1000*1000 = 16000000 * 1000 * 1000
        //这里大于等于的原因是 我们在getAmoutOut时 会丢失精度 实际取出97.27 但是取97 amount1Out小了 4000-amount1Out 就大了 
        //什么时候出现等于 amount1Out 正好取整时 
        }
    ...
    function sync() external lock {//这一步是 更新池子中实际代币数量 至 reserve（存量） 参数中
