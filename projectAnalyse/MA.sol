...
contract MA is IERC20, Ownable {
    ...
    address private fundAddress = address(0xB2802BEd7C09285e379A964a03A1148B95a5f2f7);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || balanceOf(_destroyAddress) >= 90000000 * 10 ** _decimals) {
            takeFee = false;
        }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 1; i <= 3; i++) {
            ...
            if (cur == address(0)) {
                ...
                emit Transfer(init, _destroyAddress, curTAmount);
                cur = address(0);
            }else{
                uint256 curTAmount = tAmount.div(10000).mul(rate);
                uint256 curRAmount = curTAmount.mul(currentRate);
